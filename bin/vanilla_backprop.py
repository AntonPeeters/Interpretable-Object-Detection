...
class VanillaBackprop:
    ...
    def hook_layers(self, model):
        ...
        first_layer = list(model.layers._modules.items())[0][1]
        first_layer.register_backward_hook(hook_function)

    def generate_gradients(self, output, target_class):
...
def backprop(model, output, target_class, image):
    file_name_to_export = image
    # Vanilla backprop
    VBP = VanillaBackprop(model)
    # Generate gradients
    vanilla_grads = VBP.generate_gradients(output, target_class)
