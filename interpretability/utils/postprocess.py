...
__all__ = ['GetBoundingBoxesAnchor', 'NonMaxSuppression', 'TensorToBramboxAnchor']
...
class GetBoundingBoxesAnchor(ln.data.transform.util.BaseTransform):
    ...
    def __call__(self, network_output):
        ...
        # Create array
        new_shape = network_output.shape
        new_shape = new_shape[0], new_shape[1], new_shape[2] + 1, new_shape[3]
        new_network_output = torch.zeros(new_shape)

        # copy values
        new_network_output[:, :, :-1, :] = network_output
        new_network_output[:, 0, -1, :] = 0
        new_network_output[:, 1, -1, :] = 1
        new_network_output[:, 2, -1, :] = 2
        new_network_output[:, 3, -1, :] = 3
        new_network_output[:, 4, -1, :] = 4

        # Compute class_score
        if self.num_classes > 1:
            with torch.no_grad():
                cls_scores = torch.nn.functional.softmax(new_network_output[:, :, 5:, :], 2)
            cls_max, cls_max_idx = torch.max(cls_scores, 2)
            cls_max_idx = cls_max_idx.float()
            cls_max.mul_(new_network_output[:, :, 4, :])
        else:
            cls_max = new_network_output[:, :, 4, :]
        ...
        return torch.cat([batch_num[:, None].float(), coords, scores[:, None], idx[:, None], anchors], dim=1)


class NonMaxSuppression(ln.data.transform.util.BaseTransform):
...
class TensorToBramboxAnchor(ln.data.transform.util.BaseTransform):
    ...
    def __call__(self, boxes):
        if boxes.numel() == 0:
            df = pd.DataFrame(columns=['image', 'class_label', 'id', 'x_top_left', 'y_top_left', 'width', 'height', 'confidence', 'anchor_box'])
        ...
        boxes = pd.DataFrame(boxes, columns=['image', 'x_top_left', 'y_top_left', 'width', 'height', 'confidence', 'class_label', 'anchor_box'])

        # Set column types
        boxes[['image', 'class_label']] = boxes[['image', 'class_label']].astype(int)
        boxes['id'] = np.nan
        boxes[['x_top_left', 'y_top_left', 'width', 'height', 'confidence', 'anchor_box']] = boxes[['x_top_left', 'y_top_left', 'width', 'height', 'confidence', 'anchor_box']].astype(float)
        ...
        return boxes[['image', 'class_label', 'id', 'x_top_left', 'y_top_left', 'width', 'height', 'confidence', 'anchor_box']]
